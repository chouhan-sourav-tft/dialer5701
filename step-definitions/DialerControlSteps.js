...
Then('user click on the recycle button {string} time', async (occurance) => {
  await dialerControl.clickRecycleButton(occurance);
});
...
When('user set the following values in the recycle form:', async (dataTable) => {
  ...
  let clickType = 1;
  dataTable.hashes().forEach(async (element) => {
    if(element.clickType){
      clickType = element.clickType;
    }
    settingData = {
...
Then('user click on add rule button', async () => {
  await dialerControl.addRule();
});
Then('reset dialer rule setting', async () => {
  await dialerControl.resetDialerRule();
});