...
exports.DialerControl = class DialerControl extends BaseAction {
  ...
  elements = {
    ...
    dialerName: '(//input[@name="dial-rule-name"])[last()]',
    ...
    dailerStartTime: '(//input[@name="dial-rule-startTime"])[last()]',
    dailerEndTime: '(//input[@name="dial-rule-endTime"])[last()]',
    dailerMaxtries: '(//input[@name="dial-rule-maxTries-input"])[last()]',
    deletePhoneButton: '(//div[@id="rules-phone-numbers"]//li[1]//a[@class="select2-search-choice-close"])[last()]',
    totalPhoneList: '(//div[@id="rules-phone-numbers"])[last()]//a[@class="select2-search-choice-close"]',
    phoneInput: '(//div[@id="rules-phone-numbers"])[last()]//li[@class="select2-search-field"]//input',
    recycleAddButton: '(//button[contains(@class,"recycle-rule-add-btn")]//i)[last()]',
    removePreviousRecycle: '//button[@class="recycle-rule-remove-btn"]',
    ...
    allDayCheckbox: '(//div[@id="rule-schedule"]//label[@class="checkbox"])[last()]',
    addDialRule: '.add-dial-rule',
    removeRule: '(//button[@class="dial-rule-remove-btn"])[2]',
    recycleRule: '(//button[@class="recycle-rule-remove-btn"])[2]'
  };
  ...
  /** 
    ...
    * @param {String} - occurance: click occurance
    * @returns {void} nothing
    */
  async clickRecycleButton(occurance) {
    if(occurance === 'first')
    {
  ...
  /** 
    ...
    * @param {string} - settings.clickType - click occurance
    * @returns {void} nothing
    */
  async recycleSettings(settings) {
    let recycleOutcome = `(//select[@class='select with-angle-down valid'])[${settings.clickType}]`;
    await this.selectOptionByValue(recycleOutcome, settings.callOutcome);
    let recycleInterval = `(//input[@name='recycle_rule_duration'])[${settings.clickType}]`;
    await this.forceClick(recycleInterval);
    await this.type(recycleInterval, settings.recycleInterval);
    let recycleMaxTries= `(//input[@name='recycle-max-tries-input'])[${settings.clickType}]`;
    await this.type(recycleMaxTries, settings.maxTries);
  }
  ...
  /**
    ...
    */
  async addRule() {
    await this.waitForSelector(this.elements.addDialRule);
    await this.click(this.elements.addDialRule);
  }

  /**
    * Function to reset dialer rules
    * @returns {void} nothing
    */
  async resetDialerRule() {
    if(await this.isVisible(this.elements.removeRule)){
      await this.click(this.elements.removeRule);
    }
    if(await this.isVisible(this.elements.recycleRule)){
      await this.click(this.elements.recycleRule);
    }
  }
};
