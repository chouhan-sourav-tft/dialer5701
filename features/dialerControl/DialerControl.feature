...
Feature: Dialer Control
    ...
    @6129
    Scenario: Power Dial: Reaching global max tries when alt-dialing
        When user configure Global Max tries with value '2'
        ...
        And Create Database
            | browseFile              | databaseCampaign | optionName | optionPhone1 | phone               | phoneID |
            | fixtures/database_5.csv | 1                | 0          | 1            | second,third,fourth | 2,3,4   |
        Then load the database
            | browseFile              | numOfColumnsToUpload | databaseCampaign | optionName | optionPhone1 | phone               | phoneID |
            | fixtures/database_5.csv | 1                    | 1                | 0          | 1            | second,third,fourth | 2,3,4   |
        When user navigates to dialer control menu
        Then user choose the following configurations in dialer control menu:
            | campaign             | OutboundCampaign_1   |
            | sortContactsPriority | By outcome and field |
    ...
    @5998
    Scenario: Power Dial: Close contact when alt-dialing
        When Navigate to Database Manager
        And Create Database
            | browseFile                  | databaseCampaign | optionName | optionPhone1 | phone               | phoneID |
            | fixtures/DB_GOTEST-5998.csv | 1                | 0          | 1            | second,third,fourth | 2,3,4   |
        Then user set the contact maximum tries limit
            | contactMaxTries |
            | 0               |
        Then load the database
            | browseFile                  | numOfColumnsToUpload | databaseCampaign | optionName | optionPhone1 | phone               | phoneID |
            | fixtures/DB_GOTEST-5998.csv | 1                    | 1                | 0          | 1            | second,third,fourth | 2,3,4   |
        When user navigates to voice manager
        Then user edits the campaign 'OutboundCampaign_1'
        When user navigates to dialer
        Then select the dialer type 'power'
        ...
        Then user click on the recycle button 'first' time
        When user set the following values in the recycle form:
            | callOutcome | recycleInterval | maxTries | clickType |
            | Busy        | 1m              | 3        | 1         |
        Then user click on the recycle button 'next' time
        When user set the following values in the recycle form:
            | callOutcome | recycleInterval | maxTries | clickType |
            | No Answer   | 1m              | 3        | 2         |
        Then user click on add rule button
        And user configure the folllowing rule:
            | dialerName     | Rule_2 |
            | phone          | 1      |
            | startHour      | 00:00  |
